...
// ── Shared sidebar notification counts (mirrors index.php dashboard) ──
$sidebarStats = [
    'hidden_artworks' => (int) ($conn->query("SELECT COUNT(*) FROM artworks WHERE status='hidden'")->fetch_row()[0] ?? 0),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($formData['slug'])) {
        ...
        while (true) {
            ...
            $check->bind_param('s', $formData['slug']);
            $check->execute();
            $check->store_result();
            if ($check->num_rows > 0) {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO blog_posts (title, slug, content, featured_image, author_id, status, is_featured, tags, published_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
<title>Create Blog Post — Art Bazaar Admin</title>
...
/* Image Preview */
.image-preview { margin-bottom: 12px; display: none; }
.image-preview img { max-width: 100%; max-height: 220px; border-radius: 8px; border: 1px solid var(--sand); object-fit: cover; }
.image-preview .remove-btn { display: inline-block; margin-top: 8px; font-size: 11px; color: #8b0000; cursor: pointer; text-decoration: underline; background: none; border: none; font-family: 'DM Sans', sans-serif; }
...
<aside class="sidebar">
    ...
    <a href="artworks.php" class="nav-item">
        ...
        Artworks
        <?php if ($sidebarStats['hidden_artworks'] > 0): ?>
            <span class="badge"><?= $sidebarStats['hidden_artworks'] ?></span>
        <?php endif; ?>
    </a>
    ...
    <a href="artists.php" class="nav-item">
        ...
        <?php if ($sidebarStats['pending_artists'] > 0): ?>
            <span class="badge amber"><?= $sidebarStats['pending_artists'] ?></span>
        <?php endif; ?>
    </a>
    ...
    <a href="commissions.php" class="nav-item">
        ...
        Commissions
        <?php if ($sidebarStats['new_commissions'] > 0): ?><span class="badge"><?= $sidebarStats['new_commissions'] ?></span><?php endif; ?>
    </a>
...
<header class="topbar">
    <div class="topbar-left">
        <h1>Create Post</h1>
        <div class="sub">Add a new blog entry</div>
    </div>
        ...
        <div class="card">
            <div class="form-grid">
                ...
                <div class="form-group full">
                    ...
                    <input type="text" id="slug" name="slug" class="form-input" value="<?= htmlspecialchars($formData['slug']) ?>" placeholder="auto-generated-from-title">
                    <div class="form-hint">Leave empty to auto-generate from title. Only lowercase letters, numbers, and hyphens.</div>
                </div>
                ...
                <div class="form-group full">
                    ...
                    <div class="file-input-wrapper" id="fileWrapper">
                        ...
                        <div class="file-input-text">Click to upload (JPG, PNG, WebP — Max 2MB)</div>
                    </div>
                </div>
...
function removeImage() {
    ...
    // Reset the file input so the form doesn't submit the old file
    fileInput.value = '';
}
</script>
</body>
