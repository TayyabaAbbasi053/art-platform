...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
    ...
    if ($id) {
        ...
        $toast = 'Artist and all their data deleted permanently.';
    }
}
...
/* ── Table cells ─────────────────────────────────────── */
.td-avatar { width: 36px; height: 36px; border-radius: 50%; object-fit: cover; background: var(--grey2); border: 1px solid var(--grey2); }
.td-avatar-placeholder { width: 36px; height: 36px; border-radius: 50%; background: var(--terracotta); display: flex; align-items: center; justify-content: center; font-size: 13px; color: #fff; font-weight: 600; }
.td-name { color: var(--black); font-weight: 500; }
.td-name a { color: var(--black); text-decoration: none; }
.td-name a:hover { text-decoration: underline; }
.td-email { font-size: 11px; color: var(--grey4); }
.td-stats { display: flex; gap: 10px; font-size: 11px; flex-wrap: wrap; }
.td-stats span { white-space: nowrap; }
.td-stats .num { font-weight: 600; color: var(--black); }
        ...
        <table>
            ...
            <?php foreach ($artists as $a): ?>
                <tr>
                    ...
                    <td class="td-city hide-mobile"><?= htmlspecialchars($a['city'] ?? '—') ?></td>
                    <td>
                        <div class="td-stats">
                            <span><span class="num"><?= $a['approved_count'] ?></span> approved</span>
                            <span><span class="num"><?= $a['sold_count'] ?></span> sold</span>
                            <span><span class="num" style="color:var(--amber)"><?= $a['art_count'] - $a['approved_count'] - $a['sold_count'] ?></span> other</span>
                        </div>
                    </td>
                    <td class="hide-mobile" style="font-size:11px;color:var(--grey4);white-space:nowrap"><?= date('d M Y', strtotime($a['created_at'])) ?></td>
                    <td><span class="pill <?= $a['status'] ?>"><?= ucfirst($a['status']) ?></span></td>
                    <td>
                        <div class="td-actions">
                            <?php if ($a['status'] === 'pending'): ?>
                                <form method="POST" style="display:inline"><input type="hidden" name="action" value="approve"><input type="hidden" name="id" value="<?= $a['id'] ?>"><button type="submit" class="act-btn green">Approve</button></form>
                            ...
                            <?php elseif ($a['status'] === 'blocked'): ?>
                                <form method="POST" style="display:inline"><input type="hidden" name="action" value="unblock"><input type="hidden" name="id" value="<?= $a['id'] ?>"><button type="submit" class="act-btn green">Unblock</button></form>
                            <?php endif; ?>
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal">
        ...
        <div class="modal-body">
            <p class="confirm-text">Are you sure you want to permanently delete <strong id="deleteName"></strong>? This artist has <strong id="deleteCount">0</strong> artwork(s). All their artworks, images, inquiries, and profile data will be permanently removed. This cannot be undone.</p>
        </div>
        <form method="POST">
