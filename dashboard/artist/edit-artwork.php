...
// ── Which artwork are we editing? ─────────────────────
// Present on both the initial GET (?id=) and every POST (hidden field),
// so the same lookup works regardless of request method.
 $artworkId = (int) ($_POST['artwork_id'] ?? $_GET['id'] ?? 0);
if ($artworkId <= 0) {
    header('Location: my-artworks.php');
    exit;
}

 $stmtArt = $conn->prepare("SELECT * FROM artworks WHERE id = ? AND artist_id = ?");
 $stmtArt->bind_param('ii', $artworkId, $artistId);
 $stmtArt->execute();
 $artwork = $stmtArt->get_result()->fetch_assoc();
 $stmtArt->close();

if (!$artwork) {
    header('Location: my-artworks.php?msg=notfound');
    exit;
}

// Only active/pending/hidden listings can be edited — a sold artwork has
// already been delivered/paid for, so its listing details are locked.
if (!in_array($artwork['status'], ['active', 'pending', 'hidden'], true)) {
    header('Location: my-artworks.php?msg=cannotedit');
    exit;
}

// ── Existing images for this artwork ──────────────────
 $stmtImgs = $conn->prepare("SELECT id, image_path, media_type, is_cover, sort_order FROM artwork_images WHERE artwork_id = ? ORDER BY sort_order ASC, id ASC");
 $stmtImgs->bind_param('i', $artworkId);
 $stmtImgs->execute();
 $existingImages = $stmtImgs->get_result()->fetch_all(MYSQLI_ASSOC);
 $stmtImgs->close();
...
// ── Edit Token (per page-load, prevents duplicate/multi-tab submission) ──
// Scoped per-artwork in the session so editing two artworks in two tabs
// doesn't clobber each other's token.
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $_SESSION['edit_token'][$artworkId] = bin2hex(random_bytes(16));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Prevent double/multi-tab submission — token is single-use, scoped per artwork.
    $sessionEditToken = $_SESSION['edit_token'][$artworkId] ?? null;
    $tokenMismatch = !isset($_POST['edit_token'])
        || $sessionEditToken === null
        || !hash_equals($sessionEditToken, $_POST['edit_token']);

    if ($tokenMismatch) {
        $recentSuccess = isset($_SESSION['last_edit_success_at'][$artworkId])
            && (time() - $_SESSION['last_edit_success_at'][$artworkId]) < 60;

        if ($recentSuccess) {
            // Genuine duplicate resubmission of an already-successful save.
            header("Location: my-artworks.php?msg=updated");
            exit;
        }

        $errorMsg = 'This edit session has expired. Please refresh the page and try again.';
        goto skip_processing;
    }
    // Invalidate immediately so a second tab/request with the same token can't slip through
    unset($_SESSION['edit_token'][$artworkId]);
 ...
 $is_framed              = isset($_POST['is_framed']) ? 1 : 0;
    ...
    // ── Image order/keep/add payload ──────────────────
    // The client sends one JSON array describing the final image order —
    // each entry is either {"type":"existing","id":N} for a kept image or
    // {"type":"new"} for a freshly uploaded file (files arrive in the same
    // relative order as their "new" entries in $_FILES['images']).
    $imageOrder = json_decode($_POST['image_order'] ?? '[]', true);
    if (!is_array($imageOrder)) {
        $imageOrder = [];
    }
    $existingIdsById = array_column($existingImages, null, 'id');
    $newFilesCountPosted = isset($_FILES['images']['name']) ? count(array_filter($_FILES['images']['name'], fn($n) => $n !== '')) : 0;
    $newEntriesInOrder = count(array_filter($imageOrder, fn($e) => ($e['type'] ?? '') === 'new'));
    $existingEntriesInOrder = array_filter($imageOrder, fn($e) => ($e['type'] ?? '') === 'existing');
    $totalImageCount = count($imageOrder);

    // Every "existing" entry must actually belong to this artwork.
    $invalidExistingRef = false;
    foreach ($existingEntriesInOrder as $e) {
        if (!isset($existingIdsById[(int) ($e['id'] ?? 0)])) {
            $invalidExistingRef = true;
            break;
        }
    }
    ...
    $hasExistingDigitalFile = !empty($artwork['digital_file_path']);
    ...
    if ($title === '' || (!$isShowcaseOnly && $price <= 0) || $categoryId === 0 || $totalImageCount === 0) {
        $errorMsg = $isDigitalItem
            ? 'Please fill in all required fields and keep at least one public preview image (step 1).'
            : 'Please fill in all required fields and keep at least one image.';
    } elseif ($invalidExistingRef) {
        $errorMsg = 'One of the kept images no longer belongs to this artwork. Please refresh and try again.';
    } elseif ($newEntriesInOrder !== $newFilesCountPosted) {
        $errorMsg = 'Image list did not match the uploaded files. Please refresh and try again.';
    } elseif ($size === '') {
    ...
    } elseif ($totalImageCount > 5) {
        $errorMsg = 'You can only have up to 5 images.';
    } elseif ($isDigitalItem && !$hasDigitalFile && !$hasExistingDigitalFile) {
        $errorMsg = 'Please upload the final file buyers will receive after purchase (step 2).';
    } elseif ($isDigitalItem && $hasDigitalFile && !in_array($digitalFileExt, $allowedDigitalExt)) {
        $errorMsg = 'Invalid digital file type. Allowed: ZIP, PSD, AI, PNG, JPG, PDF, GIF, MP4, MOV, WebM, MP3, WAV, M4A.';
    } elseif ($isDigitalItem && $hasDigitalFile && $_FILES['digital_file']['size'] > 300 * 1024 * 1024) {
        ...
        $stmt = $conn->prepare("
            UPDATE artworks SET
                category_id = ?, title = ?, description = ?, tags = ?, medium = ?, size = ?,
                is_framed = ?, weight_kg = ?, price = ?, city = ?, similar_work_available = ?,
                is_showcase_only = ?, delivery_type = ?
            WHERE id = ? AND artist_id = ?
        ");

        if (!$stmt) {
            error_log('Failed to prepare artwork update statement: ' . $conn->error);
            $conn->rollback();
            $errorMsg = 'Could not save changes. Please try again.';
        } else {
            $stmt->bind_param(
                'isssssiddsiisii',
                $categoryId, $title, $description, $tags, $medium, $size,
                $is_framed, $weight_kg, $price, $city, $similar_work_available,
                $isShowcaseOnly, $deliveryType, $artworkId, $artistId
            );

            if ($stmt->execute()) {

                // 1b. Handle Digital File replacement (only if a new one was posted)
                if ($isDigitalItem && $hasDigitalFile) {
                    ...
                    if (move_uploaded_file($_FILES['digital_file']['tmp_name'], $digitalDest)) {
                        ...
                        $oldDigitalPath = $artwork['digital_file_path'] ?? null;
                        ...
                        if ($oldDigitalPath) {
                            @unlink(__DIR__ . '/../../' . $oldDigitalPath);
                        }
                    } else {
                        $conn->rollback();
                        $errorMsg = 'Failed to save the new digital artwork file. Please try again.';
                        goto skip_image_loop;
                    }
                } elseif ($isDigitalItem) {
                    // No new digital file — just keep license type in sync.
                    $stmtLic = $conn->prepare("UPDATE artworks SET license_type = ? WHERE id = ?");
                    $stmtLic->bind_param('si', $licenseType, $artworkId);
                    $stmtLic->execute();
                }

                // 2. Reconcile images against $imageOrder
                $uploadDir = __DIR__ . '/../../uploads/artworks/';
                if (!is_dir($uploadDir)) {
                    mkdir($uploadDir, 0755, true);
                }

                $files = $_FILES['images'] ?? ['name' => [], 'tmp_name' => [], 'size' => [], 'error' => []];
                ...
                $stmtImgIns = $conn->prepare("INSERT INTO artwork_images (artwork_id, image_path, media_type, is_cover, sort_order) VALUES (?, ?, ?, ?, ?)");
                $stmtImgUpd = $conn->prepare("UPDATE artwork_images SET sort_order = ?, is_cover = ? WHERE id = ? AND artwork_id = ?");

                if (!$stmtImgIns || !$stmtImgUpd) {
                    error_log('Failed to prepare image statements: ' . $conn->error);
                    $conn->rollback();
                ...
                $placedCount = 0;
                $fileCursor = 0;
                $imageLoopFailed = false;

                foreach ($imageOrder as $entry) {
                    $type = $entry['type'] ?? '';
                    $sortOrder = $placedCount;
                    $isCover = ($placedCount === 0) ? 1 : 0;

                    if ($type === 'existing') {
                        $imgId = (int) ($entry['id'] ?? 0);
                        $stmtImgUpd->bind_param('iiii', $sortOrder, $isCover, $imgId, $artworkId);
                        if ($stmtImgUpd->execute()) {
                            $keptIds[] = $imgId;
                            $placedCount++;
                        }
                        continue;
                    ...
                    // Find the next non-empty uploaded file for this "new" slot
                    while ($fileCursor < count($files['name']) && $files['name'][$fileCursor] === '') {
                        $fileCursor++;
                    }
                    if ($fileCursor >= count($files['name'])) {
                        continue;
                    }
                    $i = $fileCursor;
                    $fileCursor++;
                    ...
                    if (in_array($ext, ['heic', 'heif']) && !$heicSupported) continue;
                    ...
                    if (!$isTrustedHeic && !in_array($mime, $allowedMime)) continue;
                    ...
                    if (in_array($ext, $dimensionCheckable)) {
                        ...
                        if ($imgWidth > 8000 || $imgHeight > 8000) continue;
                    }
                    ...
                    if ($mediaTypeForResize === 'image' && $ext !== 'gif' && function_exists('gd_info')) {
                        resizeImageIfNeeded($destPath);
                    }

                    $dbPath = 'uploads/artworks/' . $newName;
                    $mediaType = $mediaTypeForResize;

                    $stmtImgIns->bind_param('issii', $artworkId, $dbPath, $mediaType, $isCover, $sortOrder);
                    if ($stmtImgIns->execute()) {
                        $savedFilePaths[] = $destPath;
                        $placedCount++;
                    } else {
                        error_log('Image insert failed: ' . $stmtImgIns->error);
                        @unlink($destPath);
                    }
                }
                ...
                if ($placedCount > 0) {
                    // Remove any existing images that were dropped from the order
                    // (not referenced in $imageOrder at all) — file + row.
                    foreach ($existingImages as $ei) {
                        if (!in_array((int) $ei['id'], $keptIds, true)) {
                            $delStmt = $conn->prepare("DELETE FROM artwork_images WHERE id = ? AND artwork_id = ?");
                            $delId = (int) $ei['id'];
                            $delStmt->bind_param('ii', $delId, $artworkId);
                            $delStmt->execute();
                            $delStmt->close();
                            @unlink(__DIR__ . '/../../' . $ei['image_path']);
                        }
                    }

                    $conn->commit();
                    $_SESSION['last_edit_success_at'][$artworkId] = time();
                    header("Location: my-artworks.php?msg=updated");
                    exit;
                } else {
                    $conn->rollback();
                    ...
                    $errorMsg = 'Artwork saved but images failed to process. Please try again.';
                }
                skip_image_loop:

            } else {
                $conn->rollback();
                error_log('Artwork update failed: ' . $stmt->error);
                $errorMsg = 'Could not save changes. Please try again.';
            }
        }
    }
...
/* ── Edit context strip (title + status of the artwork being edited) ── */
.edit-context { display: flex; align-items: center; gap: 10px; margin: -10px 0 20px; flex-wrap: wrap; }
.edit-context-title { font-family: 'Playfair Display', serif; font-size: 17px; color: var(--ink); }
...
/* ── Reorder controls on preview tiles ────────────────── */
.reorder-controls { position: absolute; bottom: 4px; right: 4px; display: flex; gap: 3px; }
.reorder-btn {
    width: 20px; height: 20px; border-radius: 50%; border: none; padding: 0;
    background: rgba(12,63,48,.88); color: var(--bg); cursor: pointer;
    display: flex; align-items: center; justify-content: center; font-size: 12px; line-height: 1;
}
.reorder-btn:hover:not(:disabled) { background: var(--ink); }
.reorder-btn:disabled { opacity: .3; cursor: default; }
@media (pointer: coarse) {
    .reorder-btn { width: 26px; height: 26px; font-size: 15px; }
}
</style>
</head>
<body>
...
<div class="content">
    ...
    <div class="section-title">Edit Artwork</div>

    <div class="edit-context">
        <span class="edit-context-title"><?= htmlspecialchars($artwork['title']) ?></span>
        <span class="pill <?= htmlspecialchars($artwork['status']) ?>"><?= ucfirst($artwork['status']) ?></span>
    </div>
    ...
    <input type="hidden" name="edit_token" value="<?= htmlspecialchars($_SESSION['edit_token'][$artworkId] ?? '') ?>">
    <input type="hidden" name="artwork_id" value="<?= (int) $artworkId ?>">
    <input type="hidden" name="image_order" id="imageOrderInput" value="">
        ...
        <div class="card">
                ...
                <div class="field-group" style="margin-bottom:0;">
                    ...
                    <div class="delivery-options">
                        <label class="delivery-option">
                            <input type="radio" name="delivery_type" value="physical" <?= $artwork['delivery_type'] === 'physical' ? 'checked' : '' ?>>
                        ...
                        <label class="delivery-option">
                            <input type="radio" name="delivery_type" value="digital" <?= $artwork['delivery_type'] === 'digital' ? 'checked' : '' ?>>
                            <span class="delivery-card">
            ...
            <div class="upload-section" id="digitalFileGroup" style="display:none;">
                ...
                <div class="field-group">
                    <label for="digitalFileInput">Digital artwork file <?= $artwork['digital_file_path'] ? '' : '<span>*</span>' ?></label>
                    <?php if ($artwork['digital_file_path']): ?>
                        <div class="file-summary" style="display:flex;">
                            <span>✓ Current file: <?= htmlspecialchars(basename($artwork['digital_file_path'])) ?><?= $artwork['digital_file_format'] ? ' (' . htmlspecialchars(strtoupper($artwork['digital_file_format'])) . ($artwork['digital_file_size_bytes'] ? ', ' . htmlspecialchars(number_format($artwork['digital_file_size_bytes'] / 1048576, 1)) . ' MB' : '') . ')' : '' ?> — this is what buyers currently receive. Choose a new file below only to replace it.</span>
                        </div>
                    <?php endif; ?>
                ...
                <div class="field-group">
                    ...
                    <select name="license_type" id="licenseInput" class="field-input">
                        ...
                        <option value="Personal Use" <?= $artwork['license_type'] === 'Personal Use' ? 'selected' : '' ?>>Personal Use — buyer can use it for themselves, not resell or use commercially</option>
                        <option value="Commercial Use" <?= $artwork['license_type'] === 'Commercial Use' ? 'selected' : '' ?>>Commercial Use — buyer can use it for business/commercial purposes</option>
                        <option value="Extended/Exclusive" <?= $artwork['license_type'] === 'Extended/Exclusive' ? 'selected' : '' ?>>Extended / Exclusive — buyer gets full or exclusive rights</option>
                    </select>
            ...
            <div class="form-grid full" style="margin-top: 24px;">
                <div class="field-group">
                    ...
                    <input type="text" name="title" class="field-input" placeholder="e.g. The Blue Horizon" value="<?= htmlspecialchars($artwork['title']) ?>" required>
                </div>
            </div>
                ...
                <div class="field-group">
                    ...
                    <select name="category" class="field-select" id="categorySelect" required>
                        ...
                        <?php foreach ($categories as $cat): ?>
                            <option value="<?= $cat['id'] ?>" data-slug="<?= htmlspecialchars($cat['slug']) ?>" <?= (int) $cat['id'] === (int) $artwork['category_id'] ? 'selected' : '' ?>><?= htmlspecialchars($cat['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="field-group">
                    <label>Medium <span>*</span></label>
                    <input type="text" name="medium" id="mediumInput" class="field-input" placeholder="e.g. Oil on Canvas" value="<?= htmlspecialchars($artwork['medium']) ?>" required>
                </div>
            </div>
            ...
            <div class="form-grid">
                <div class="field-group">
                    ...
                    <input type="text" name="size" id="sizeInput" class="field-input" placeholder="e.g. 24 x 36 inches" value="<?= htmlspecialchars($artwork['size']) ?>" required>
                ...
                <div class="field-group">
                    ...
                    <input type="number" name="price" id="priceInput" class="field-input" placeholder="e.g. 25000" min="1" value="<?= $artwork['price'] !== null ? htmlspecialchars($artwork['price']) : '' ?>" required>
                    <label class="checkbox-row">
                        <input type="checkbox" name="is_showcase_only" id="showcaseCheckbox" value="1" <?= $artwork['is_showcase_only'] ? 'checked' : '' ?>>
            ...
            <div class="form-grid" id="weightGroupRow">
                <div class="field-group" id="weightGroup">
                    ...
                    <input type="number" name="weight_kg" id="weightInput" class="field-input" placeholder="e.g. 1.5" min="0.1" step="0.1" value="<?= htmlspecialchars($artwork['weight_kg']) ?>" required>
            ...
            <div class="form-grid">
                <div class="field-group">
                    ...
                    <div class="city-search-wrap">
                        <input type="text" class="field-input city-search-input" id="citySearchInput" placeholder="Search or type your city..." autocomplete="off" value="<?= htmlspecialchars($artwork['city']) ?>">
                        <input type="hidden" name="city" id="cityHidden" required value="<?= htmlspecialchars($artwork['city']) ?>">
            ...
            <div class="form-grid full">
                <div class="field-group">
                    ...
                    <textarea name="description" id="descriptionInput" class="field-textarea" placeholder="Tell the story behind this artwork, techniques used, inspiration..."><?= htmlspecialchars($artwork['description']) ?></textarea>
                </div>
            </div>
            ...
            <div class="form-grid full">
                <div class="field-group">
                    ...
                    <input type="text" name="tags" class="field-input" placeholder="e.g. floral, abstract, blue, Lahore, miniature, portrait, gift" value="<?= htmlspecialchars($artwork['tags']) ?>">
            ...
            <div class="toggle-row">
                ...
                <label class="toggle-switch">
                    <input type="checkbox" name="is_framed" value="1" <?= $artwork['is_framed'] ? 'checked' : '' ?>>
                    <span class="toggle-slider"></span>
                </label>
            </div>
...
// Images already saved on the server for this artwork, and whether a
// private digital file already exists — both drive how the picker below
// seeds itself and whether the digital-file input is required.
const EXISTING_IMAGES = <?= json_encode(array_map(function ($img) {
    $ext = strtolower(pathinfo($img['image_path'], PATHINFO_EXTENSION));
    $kind = $img['media_type'] === 'video' ? 'video' : ($img['media_type'] === 'audio' ? 'audio' : ($ext === 'gif' ? 'gif' : 'raster'));
    return [
        'id'   => (int) $img['id'],
        'url'  => '../../' . $img['image_path'],
...
}, $existingImages), JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
const HAS_EXISTING_DIGITAL_FILE = <?= $artwork['digital_file_path'] ? 'true' : 'false' ?>;
...
// A showcase-only piece has no price (it's portfolio-only), so don't force
// the artist to type one just to save an otherwise unrelated change.
function updateShowcaseState() {
    const showcaseOnly = showcaseCheckbox.checked;
    priceInput.required = !showcaseOnly;
    if (priceRequiredMark) priceRequiredMark.style.display = showcaseOnly ? 'none' : '';
}
showcaseCheckbox.addEventListener('change', updateShowcaseState);
...
function syncFileInput() {
    ...
    // Untouched existing images have no local file (and don't need one —
    // the server already has them); only items being uploaded belong here.
    items.forEach(item => { if (!item.existingId && item.out) dt.items.add(item.out); });
    fileInput.files = dt.files;
}

// Tells the server the final order, and for each slot whether to keep an
// existing image by id or expect the next uploaded file to fill it.
function buildImageOrder() {
    imageOrderInput.value = JSON.stringify(items.map(item =>
        item.existingId ? { type: 'existing', id: item.existingId } : { type: 'new' }
    ));
}
...
async function getBitmap(item) {
    if (!item.bitmap) {
        if (!item.orig) {
            // Existing image, never pulled down before — fetch it once so it
            // can be redrawn with the current protection settings. Same-origin,
            // so this is a plain fetch, no CORS involved.
            const resp = await fetch(item.existingUrl);
            const blob = await resp.blob();
            item.orig = new File([blob], item.name, { type: blob.type || 'image/jpeg' });
        }
        item.bitmap = await createImageBitmap(item.orig);
    }
    return item.bitmap;
}
...
function needsProcessing(item) {
    if (item.existingId && !item.orig) {
        // Still the untouched file already on the server — only worth
        // fetching it back down if protection needs to be (re)applied.
        return !!(getProtection() && item.kind === 'raster');
    }
...
function processItem(item) {
    ...
    const job = (async () => {
        ...
        try {
            ...
            console.warn('Preview processing failed, using original file:', item.orig?.name || item.name, err);
        ...
        // It now has a freshly rendered file behind it, so on save this
        // should be uploaded as a replacement, not kept by reference. If
        // reprocessing produced nothing usable (e.g. the fetch itself
        // failed), leave existingId alone — the server still has the file.
        if (item.existingId && item.out) item.existingId = null;
...
// Swaps a tile with its neighbor — the first tile is always the cover, so
// this is how the artist picks a different cover or just reorders the set.
...
function renderPreviews() {
    ...
    items.forEach((item, index) => {
        // Untouched existing images have no local file yet — show them
        // straight from the server instead of manufacturing an object URL.
        let mediaUrl;
        if (item.out) {
            mediaUrl = URL.createObjectURL(item.out);
        ...
        if (item.kind === 'video') {
            ...
            media.textContent = '🎵 ' + (item.out ? item.out.name : item.name);
        } else {
        ...
        // Tell the artist what state each tile is in: still the file
        // already on the server, or freshly (un)protected just now.
        if (!item.processing) {
        ...
        // Reorder — dragging/tapping a tile all the way to the front is
        // also how the artist picks a different cover image.
        ...
        reorder.appendChild(leftBtn);
        reorder.appendChild(rightBtn);
...
function updateProtectPreview() {
    ...
    if (shown) {
        if (shown.out) {
            ...
            protectPreviewImg.src = shown.existingUrl;
        }
...
function updateDigitalFileVisibility() {
    ...
    // Step 2 (final file + license) only exists for digital listings. If a
    // file is already saved, a new one is optional — the artist is only
    // replacing it, not starting from scratch.
    digitalFileGroup.style.display = isDigital ? 'block' : 'none';
    digitalFileInput.required = isDigital && !HAS_EXISTING_DIGITAL_FILE;
...
uploadForm.addEventListener('submit', function (e) {
    ...
    // Untouched existing images have no local file and nothing to check —
    // the server already accepted them once.
